...
type Props = BaseProps | (BaseProps & (
  ClassNameRenderers | IndependentRenderers
));

const IntlMessage: React.StatelessComponent<Props> = (props) => {
  const baseProps = { id: props.id, ...("values" in props ? {values: props.values} : {})};
  return (
    <FormattedMessage {...baseProps} >
      {(message: string[]) => {
        ...
        const tagName = "tagName" in props && props.tagName !== undefined ?
          props.tagName : "span";
